...
internals.Dispenser.prototype._write = function (buffer, encoding, next) {
    ...
    if (this._stream &&
        this._stream._backpressures) {

        this._stream._backpressures.push(next);
        return;
    }
    ...
    if (this._state === internals.state.preamble) {
        ...
        if (this._stream) {
            if (!this._stream.write(chunk) &&
                !this._stream._backpressures) {                                         // Stream payload
...
internals.Dispenser.prototype._onLineEnd = function () {
    ...
    if (this._state === internals.state.header) {
        ...
        if (disposition.filename) {
            ...
            this._stream._backpressures = null;
            this._stream.on('drain', internals.onDrain);
...
internals.onDrain = function () {

    for (var i = 0, il = this._backpressure.length; i < il; ++i) {
        this._backpressure[i]();
    }

    this._backpressures = null;
};
